...
namespace rmq {
template <bool Bit>
SelectTablePacked<Bit>::SelectTablePacked(BitArray const &ba,
                                          size_t nones)
...
Select<Bit>::Select(BitArray const &ba, Rank9 const &ba_rank)
    : ba_(ba), summary_(make_summary_ba<Bit>(ba_)),
      delimiter_(
          make_delimiter_ba<Bit>(ba_, ba_rank.rank<Bit>(ba.size()))),
      ba_rank_(ba_rank), summ_sel1_(summary_),
      deli_rank_(delimiter_) {}
